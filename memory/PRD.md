...
- Se aplicó el cambio explícitamente pedido: altura de `h-72 md:h-[420px]` → `h-96 md:h-[520px] lg:h-[600px]` en `StadiumSection`. Verificado con screenshot temporal (imagen de prueba de Unsplash) en mobile y desktop — ancho completo, sin gap arriba, más alto. Se restauró `stadium.image_url` a `""` (estado real de producción, sin imagen configurada aún por el admin).
