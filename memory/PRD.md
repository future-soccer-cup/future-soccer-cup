...
### 2026-08-31 — Iter76: Tagline "Torneo Internacional" (v4 — quitado el `scaleX` que deformaba las letras)
- El usuario reportó que el `scaleX` (v3) hacía que la letra cursiva se viera "horrible" (trazos deformados, curvas estiradas de forma antinatural — normal en fuentes script, que no toleran bien el estiramiento horizontal puro).
- Fix: se quitó `scaleX` del componente `StretchedTagline`. Ahora usa `letter-spacing` calculado dinámicamente (mismo mecanismo de medir contenedor vs. ancho natural del texto), pero con un TOPE (`fontSize * 0.16`) para no separar tanto las letras que se rompa la conexión visual del script. El contenedor sigue siendo `flex-1` (mismo ancho que el menú), pero el texto se centra dentro de ese espacio en vez de forzar que toque el borde exacto — prioriza que se vea bien sobre el ajuste pixel-perfecto. Tamaño de fuente subido a `clamp(2.2rem, 3.8vw, 4rem)` para que ocupe más espacio de forma natural (sin depender solo del letter-spacing).
