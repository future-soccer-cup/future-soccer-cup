...
- Verificado con curl (test temporal con 2 logos de Unsplash) + screenshot con scroll simulado: confirmado que antes de entrar al viewport el wrapper tiene `opacity:0` y `transform: translateX(-32px)`, y tras el scroll pasa a `opacity:1`/posición final centrada. Datos de producción restaurados a `logo_url: ""` (sin logos configurados, como estaban antes de la prueba).
