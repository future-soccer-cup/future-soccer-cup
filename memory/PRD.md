...
### 2026-09-01 — Iter81: Palabra "PREMIER" en Natura Script en Estadísticas + texto un poco más grande
- `DatosEstadisticas.jsx`: en las pestañas del selector de evento ("Premier Pares"/"Premier Impares"), la palabra "PREMIER" ahora se detecta por texto (case-insensitive) y se renderiza en cursiva Natura Script (antes todo el label iba en Plane Crash grunge). El resto de la etiqueta (PARES/IMPARES/FESTIVAL) sigue en Plane Crash. Tamaño del botón subido de `text-sm md:text-base` a `text-base md:text-lg`.
- El bloque "DICIEMBRE / PREMIER" (título derecho de la grilla de categorías, `stats-event-title-word`) ya usaba Natura Script — se aumentó su tamaño de `clamp(1.8rem,3.2vw,2.8rem)` a `clamp(2.2rem,3.8vw,3.4rem)`.
