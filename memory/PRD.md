...
### 2026-09-01 — Iter82: Bug de fondo encontrado y corregido — Natura Script nunca cargaba el archivo real en TODO el sitio
- El usuario insistió en que "PREMIER" no tenía la fuente Natura Script real. Se comprobó con `document.fonts.check("16px 'Natura Script'")` → `false`. Causa raíz: había DOS declaraciones `@font-face` para 'Natura Script' (y 'Plane Crash'): la correcta en `/public/fonts.css` (con `url('/fonts/NaturaScript.otf')` real), y una segunda en `src/index.css` que solo tenía `src: local(...)` (sin url) — al ser bundleada por webpack y cargarse después, esta segunda declaración pisaba a la primera, y como ningún visitante tiene "Natura Script" instalado localmente, el navegador SIEMPRE caía al fallback (Dancing Script/Allura) sin que se notara a simple vista.
- Fix: se eliminó la declaración duplicada/rota de `src/index.css`, dejando `/public/fonts.css` como única fuente de verdad. Confirmado con `document.fonts.check(...)` → `true` tras el fix. Esto corrige la fuente cursiva real en TODO el sitio (no solo "PREMIER"): Navbar "Torneo Internacional", Home hero, LoginModal, TeamRegister "team fsc", etc. — todos ahora cargan el archivo `.otf` real en vez del fallback silencioso.
