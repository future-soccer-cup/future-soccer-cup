...
### 2026-09-10 — Iter118: Footer — "TOMAMOS UN" definitivamente en una sola línea (fix final, verificado con testing_agent)
- El usuario aclaró la referencia exacta: 4 líneas — "¿Y SI NOS" / "TOMAMOS UN" / "CAFECITO" / "JUNTOS?" ("UN" debe ir junto a "TOMAMOS", no solo).
- **Causa raíz real**: el valor en BD (`home_settings.footer_heading`) ya tenía "TOMAMOS UN" en la misma línea (sin `\n` entre ellas) desde el Iter117, pero el `<h2>` del footer no tenía `whitespace-nowrap` en cada `<span className="block">` — a `lg:grid-cols-2` (columna ~640px) y `fontSize` de hasta 72px, "TOMAMOS UN" se desbordaba y el navegador lo envolvía visualmente en 2 líneas aunque fuera un solo string.
- Fix en `Footer.jsx`: grid cambiado a `lg:grid-cols-[3fr_2fr]` (más ancho para el heading), `fontSize` reducido a `clamp(30px, 4.2vw, 64px)`, cada línea con `className="block lg:whitespace-nowrap"` (nowrap SOLO en desktop ≥1024px; en mobile se deja wrap natural para no desbordar pantallas angostas).
- Verificado con `testing_agent` (`/app/test_reports/iteration_68.json`): 100% frontend, 0 bugs — 4 líneas exactas en desktop 1920px y mobile 390px, sin overflow horizontal, sin recorte, sin overlap del "¿", resto del footer (WhatsApp/email/redes/TikTok/"Somos más que un Torneo"/copyright) sin regresiones, presente en todas las páginas públicas salvo /registro.
...
- **3 correcciones adicionales**: (1) animación de eco de MARCADOR/MENTALIDAD ahora arranca desde -90px (antes -50px), cae desde "un poco más arriba" como pidió el usuario. (2) El símbolo "¿" desaparecía en la fuente Plane Crash (glifo invisible, igual que pasaba con "ñ") — `renderPlaneCrash` en `designSystem.js` ahora dibuja "¿" explícitamente con la fuente de respaldo (Anton), igual que ya hacía con "ñ". (3) El titular del footer ("¿Y SI NOS TOMAMOS UN CAFECITO JUNTOS?") se guardaba como una sola línea larga y Footer.jsx la partía con word-wrap automático (líneas desalineadas, ej. "UN" solo). Fix: el campo ahora usa saltos de línea explícitos (`\n`) — textarea en el admin, default y valor en BD actualizados a 4 líneas — reproduciendo el diseño original.
- **Ajuste fino tras feedback**: el usuario mostró la referencia real — son 5 líneas ("¿Y SI NOS"/"TOMAMOS"/"UN"/"CAFECITO"/"JUNTOS?", "UN" solo) y el "¿" debe verse con la MISMA textura grunge que el resto (mi fix anterior lo dibujaba con la fuente de respaldo Anton, que se veía distinta/lisa). Fix definitivo: "¿" se dibuja como un "?" normal (mismo glifo de Plane Crash, con textura) rotado 180° via CSS `transform`, en vez de cambiar de fuente — visualmente es un "¿" con la textura correcta. Default/valor en BD actualizado a 5 líneas. `text-left` explícito agregado al `<h2>` del footer.
- **Fix overlap del "¿"**: la rotación 180° del glifo "?" se centraba en la caja de línea completa (alta), no en la tinta del carácter, dejando el "¿" invadiendo la línea de abajo ("TOMAMOS"). Agregado `translateY(0.22em)` + `lineHeight:1` de compensación tras la rotación. Verificado sin overlap.
...
- **Fix dirección**: el usuario indicó que el eco debía quedar arriba (no abajo) del texto principal, con solo 3 copias (0.4/0.2/0.1) y sin desplazar/cortar "MARCADOR". Cambiado `offset` a valores negativos (-10/-20/-30px, hacia arriba) y removida la 4ta copia (0.6). Texto principal sin cambios de posición (las copias son `absolute`, no afectan el flujo).
- **Extendido a Noticias**: mismo efecto (eco hacia arriba, sin watermark estático) aplicado a "MENTALIDAD" en `Noticias.jsx` (antes tenía el mismo patrón de watermark estático que Estadísticas). "Fútbolera" sin cambios.
...
- **`deployment_agent` encontró y se corrigió un BLOCKER real** (no relacionado al video): `seed_demo_inventory()` en `server.py` hacía `db[coll].drop()` de las colecciones `hotels/transports/tours` en CADA reinicio del backend (cada redeploy/restart de pod), borrando datos permanentemente. Se quitaron esos `drop()`. También se cambió `CORS_ORIGINS` a `"*"` en `backend/.env` (Starlette refleja el origin exacto cuando hay cookies, así que sigue funcionando con `allow_credentials=True`).
- Pendiente: usuario debe confirmar que la subida de un video real de 140MB ya funciona en producción (requiere una nueva build/deploy para que el fix llegue a producción).
...
### 2026-09-09 — Iter114: Regresión del Iter112/113 — las flechas quedaron muy separadas del título en tablet/desktop
- El usuario mostró captura comparativa: las flechas quedaron pegadas a los bordes de la sección (muy lejos del texto) en vez de cerca del título como el diseño original, en tablet y escritorio.
- **Causa raíz**: el cambio a `grid-cols-[auto_1fr_auto]` (Iter112) hacía que las columnas de flecha se redujeran a su ancho de contenido y quedaran ancladas en los bordes extremos de la sección `max-w-7xl`, mientras la columna central (1fr) absorbía todo el resto — separando mucho las flechas del texto en pantallas anchas (no se notaba en mobile porque ahí es donde se necesitaba el espacio extra).
- **Fix**: el grid `auto_1fr_auto` ahora solo aplica en mobile (`<640px`); desde `sm:` (≥640px) se vuelve a las 3 columnas iguales originales (`sm:grid-cols-3`) que reproducen exactamente la posición de flechas del diseño de referencia. El tamaño de fuente de "FESTIVAL"/"Premier" se ajustó a valores más conservadores (`clamp(1.3rem,4.8vw,3.6rem)` y `clamp(1.6rem,6.2vw,4.8rem)`) para que el texto en `whitespace-nowrap` siempre entre en su columna de 1/3, en cualquier ancho de pantalla (probado en 320/390/640/768/1024/1280px sin overlap, wrap raro ni texto faltante).
- Verificado con capturas de pantalla en 320, 390, 768 y 1280px (Festival y Premier): las flechas ahora quedan cerca del título como en el diseño original/deseado, sin superposición ni desborde en ningún breakpoint.
- **Corrección final**: el usuario reportó que aún se veía cortado en tablet/escritorio ("DICIEMBRE"/"FESTIVAL" recortados a la mitad por `overflow-x-hidden`, pegados a la flecha). Causa: `sm:grid-cols-3` (columnas iguales) seguía siendo muy angosto para palabras largas sin espacios como "DICIEMBRE" (9 letras, no puede envolver, por lo que se recortaba). Fix definitivo: `sm:grid-cols-[1fr_2.2fr_1fr]` (columna central 2.2x más ancha que las de flecha, en vez de igual) + valores de fuente originales restaurados. Verificado con `testing_agent` de forma independiente (no solo visual, sino con medición de bounding-box): 16/16 combinaciones (8 anchos: 320/375/640/768/1024/1280/1440/1920px × 2 pestañas Festival/Premier) sin recorte (scrollWidth==clientWidth) y sin overlap de flechas. `/app/test_reports/iteration_67.json`.
- **4ta corrección — causa raíz real encontrada**: el usuario reportó que seguía cortado ("DICIEMBRE" con la "E" final tapada). Medí el ancho REAL de la fuente "Plane Crash" con `getBoundingClientRect` (no estimación): ~0.83 de ancho por carácter (yo había asumido 0.65, un 27% de subestimación), más el `letterSpacing:0.02em` de "FESTIVAL" que dejaba el margen de seguridad en ~0px en tablet — cualquier variación de renderizado lo hacía desbordar y recortarse. Fix: fuente de "FESTIVAL"/"PREMIER" reducida a `clamp(1.15rem,5.2vw,4.2rem)` con margen real de 15-18% en todos los breakpoints. Verificado programáticamente (`scrollWidth <= clientWidth`, con `document.fonts.ready` esperado) en 20 combinaciones (10 anchos 320-1920px × Festival/Premier): overflow=0 en TODAS.
- **Aclaración final (no era bug de código)**: el usuario insistió que en Edge/Windows seguía viendo una barra gris vertical con flechas arriba/abajo junto a "OCTUBRE FESTIVAL"/"DICIEMBRE Premier", incluso al maximizar la ventana. Analizando la imagen enviada en detalle: coincide exactamente con la barra de scroll nativa/overlay de Windows Edge (para desplazar la página completa, que es más alta que la pantalla) — no un elemento roto del código. En Windows con escalado de pantalla (125%/150%, común en laptops), el ancho efectivo en píxeles CSS de la ventana maximizada puede quedar muy cerca de los ~1280px que usaba `max-w-7xl`, dejando poco margen entre el chevron derecho y el borde real de la ventana (donde vive esa barra). Medida preventiva (no por bug, para dar más aire): `eventos-tabs` y `eventos-title-section` reducidos a `max-w-6xl` con más padding lateral (`px-4 sm:px-10 lg:px-16`, antes `px-4 sm:px-6 lg:px-8`), ~130-190px extra de margen respecto al borde de cualquier ventana. Reverificado sin overflow en 320-1920px.
- **Regresión de alineación (revertida)**: el cambio de `max-w-7xl`→`max-w-6xl` de la corrección anterior movió las cajas FESTIVAL/PREMIER hacia el centro, rompiendo su alineación original (FESTIVAL a la izquierda, PREMIER a la derecha, como en el diseño de referencia del usuario). Revertido a `max-w-7xl` + padding original (`px-4 sm:px-6 lg:px-8`) en `eventos-tabs` y `eventos-title-section`. También se quitó `overflow-x-hidden` de los 4 contenedores de texto (ya no es necesario: los márgenes de seguridad de fuente ya garantizan 0 overflow, y `overflow-x-hidden` convierte al div en "scroll container" a nivel de spec CSS aunque esté clippeado, lo cual podría explicar por qué Edge en la máquina del usuario mostraba una barra de scroll junto al texto — el usuario insiste en que es "el scroll del texto", no el de la página general).
...
- **Fix**: `Eventos.jsx` — `EventTitleSection` ahora usa `grid-cols-[auto_1fr_auto]` (columna central toma todo el espacio restante) + `ChevronStack` envuelto en `scale-[0.5] sm:scale-75 md:scale-100` para reducirlo en móvil; texto de "FESTIVAL"/mes con `whiteSpace:nowrap` para evitar wrap letra-por-letra. Tabs FESTIVAL/PREMIER/EVENTOS con `fontSize` mínimo reducido + `whiteSpace:nowrap` + `overflow-hidden`. `CategoryBlock` con `overflow-hidden`, `whitespace-nowrap` y fuentes más pequeñas en mobile. `PremierCategories` cambiado a `grid-cols-1 sm:grid-cols-2` (PARES/IMPARES apilados verticalmente en móvil, lado a lado desde `sm:`). `FSCHistorySection.jsx` — `minHeight` inline reemplazado por clases Tailwind `md:min-h-[...]` (sin altura mínima forzada en móvil).
- Verificado con `testing_agent` a 320px y 390px (Eventos + Nosotros) y regresión desktop 1920x800: los 5 bugs quedaron resueltos, sin nuevos issues críticos. `/app/test_reports/iteration_66.json`.
- Notas menores no bloqueantes detectadas por el testing agent (pendientes, no reportadas por el usuario): 2 requests 401 en consola en páginas públicas (posible llamada auth innecesaria), y el hero de escritorio de /eventos se ve en blanco (hay un `hero_video_url` configurado en el CMS pero no se renderiza visualmente — a investigar si el usuario lo reporta).
...
- **Corrección adicional en el mismo Iter111**: el usuario aclaró que quería la imagen OCULTA por completo en tablet, no apilada debajo del formulario (así seguía apareciendo el león empujando el formulario hacia abajo). `LoginModal.jsx` corregido: el panel de imagen ahora solo se renderiza cuando `isDesktop` es verdadero (≥1024px) — en mobile/tablet no se monta en absoluto. `TeamRegister.jsx`/`ForgotPassword.jsx`/`ResetPassword.jsx` ya la ocultaban correctamente con `hidden lg:flex`. Verificado en 768px con las 3 vistas (login, forgot con código, reset): sin imagen, formulario completo y accesible sin scroll extra.
...
- Aplicado a los heros de `Eventos.jsx` (solo cuando NO hay video, el video mantiene su alto fijo de siempre), `Noticias.jsx` y `DatosEstadisticas.jsx`.
- **Bug encontrado por testing_agent**: cuando la foto es muy vertical y el alto calculado se topa con el `maxHeight` (clamp), la sección ya no coincide con la proporción real de la foto — con `object-cover` eso volvía a recortar cabezas/pies. Corregido cambiando las 3 imágenes hero de `object-cover` a `object-contain` (el overlay de color ya existente disimula las franjas laterales cuando aparecen).
- `Noticias.jsx`: tarjetas de categoría, miniaturas del modal de noticias y galería de detalle de noticia — cambiadas de `object-cover` a `object-contain` con fondo `bg-slate-900` de respaldo (antes `bg-slate-100`).
- `GalleryCarousel.jsx` (Escenarios Deportivos/Premiación en Eventos) y galería "Finales" de `Home.jsx` — cambiadas de `object-cover` a `object-contain` (el fondo de color de cada slot ya actúa como marco si la foto no calza exacto).
- Hints de tamaño de imagen actualizados en `AdminHomeSettings.jsx` y `AdminGallery.jsx` para reflejar que ya no se recorta y sugerir proporciones ideales.
- **Explícitamente NO tocado** (son recortes intencionales de diseño, no bugs): escudos de clubes (ya usan `object-contain`), avatares circulares de jugadores, foto tipo carnet/cédula del jugador (formato ID fijo), Home hero principal y `FSCHistorySection` (collage de fotos flotantes con texto posicionado con precisión — cambiar esto rompería la composición).
- **Verificado con `testing_agent`** (iteration_64.json): encontró el bug de `object-cover`+clamp arriba descrito (HIGH priority en Noticias/Estadísticas, MEDIUM en Eventos) — corregido y reverificado con screenshot en 1920px, ya no se recortan cabezas/pies en ningún hero.
- Issue preexistente detectado (NO de esta sesión, no corregido): título "MENTALIDAD" en el hero de Noticias se desborda un poco en mobile 390px (clamp de fuente muy grande). Queda en backlog, no se tocó sin pedirlo explícitamente el usuario.

### 2026-09-08 — Iter102: Imágenes de INGRESO/REGISTRO — ancho ahora se ajusta EXACTO a la proporción real (sin recortar, sin estirar, sin espacios vacíos)
- El usuario confirmó que Iter101 (object-contain) arregló el ALTO pero no el ANCHO: al no coincidir la proporción real de la foto con la del panel fijo (50%/40% de la tarjeta), quedaban franjas vacías a los lados. Pidió explícitamente "ajustar la imagen" — descartó la opción de fondo desenfocado tipo Spotify/Netflix.
- **Solución implementada**: nuevo hook compartido `/app/frontend/src/hooks/useAutoFitSideImage.js` que mide (via `ResizeObserver` + `onLoad` de `naturalWidth/naturalHeight`) el alto real del panel y la proporción real de la imagen, y calcula el ANCHO EXACTO en px que debe tener el contenedor para que la imagen llene height×ratio sin sobrar ni faltar espacio (con un tope `maxWidthRatio` sobre el ancho del contenedor padre, para casos extremos de fotos muy panorámicas).
- **Bug encontrado en el propio fix**: la primera versión usaba `useRef` + `useEffect([])`, pero el panel vive dentro de un modal que se monta oculto (`if (!open) return null`) — el efecto corría una sola vez con el ref en `null` y el `ResizeObserver` nunca se adjuntaba. Corregido usando un **ref-callback** (`useState` + `useCallback`) que dispara el efecto cada vez que el nodo real se monta/desmonta.
- `LoginModal.jsx`: tarjeta cambiada de `grid grid-cols-2` a `flex flex-col md:flex-row`. En mobile (apilado) la imagen es simplemente `w-full h-auto` (ancho completo, alto automático según proporción — sin JS). En desktop, el panel usa el hook: ancho exacto en px (`flex-shrink-0`), imagen `h-full w-full object-contain` (contain ya no recorta porque el contenedor mide EXACTO lo que la imagen necesita).
- `TeamRegister.jsx`: mismo patrón — contenedor general de `grid` a `flex flex-col md:flex-row`; columna imagen (`hidden md:flex`, sin versión mobile porque ya estaba oculta en mobile) usa el mismo hook.
- **Verificado con `bounding_box()` (Playwright) + screenshots**, no con testing_agent (cambio acotado a 2 archivos + 1 hook, verificación propia suficiente):
  - Desktop 1920px: `login-modal-image` = 471×602px (tope 46% de 1024px de tarjeta) llenando el panel completo con la foto del león sin cortes ni bordes vacíos. `tr-image-side` = 668×836px (tope 42% del ancho restante) mismo resultado.
  - Mobile 390px: `login-modal-image` = 358×447.5px (ancho completo del modal, alto automático = 358×(1000/800) exacto, sin recorte).
  - Estado sin imagen configurada (placeholder "Imagen no configurada"): se mantiene un ancho mínimo de fallback (200px Login / 320px Registro) para que no colapse a 0px.
  - Datos de producción (`auth_login_image_url`/`auth_register_image_url`) restaurados a sus valores originales tras la prueba (se usó una imagen de prueba temporal 800×1000 vía PUT /api/home-settings, luego revertida).

### 2026-09-07 — Iter101: Imágenes de INGRESO/REGISTRO ya no se recortan — cambiadas a object-contain
- El usuario mostró que la imagen de KOW en el modal de Login y en la página de Registro de equipo se veía recortada (cabeza y pies cortados) porque usaba `object-cover` en un contenedor con proporción distinta a la imagen (800×1000/800×1200).
- Fix: `LoginModal.jsx` (panel derecho) y `TeamRegister.jsx` (`tr-image-side`) cambiados de `object-cover` a `object-contain` — la imagen ahora SIEMPRE se muestra completa (con relleno del fondo oscuro a los lados si no coincide la proporción, en vez de recortar). Hints del Admin (`AdminHomeSettings.jsx`) actualizados de "Se recorta cover" a "Se muestra completa, sin recortar".
- Verificado con imagen de prueba (retrato de jugador): cabeza y pies visibles completos, sin cortes. Datos de producción restaurados a `""` (sin imagen configurada, como estaban).
