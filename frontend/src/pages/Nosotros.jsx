    ...
    <div data-testid="nosotros-page">
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid lg:grid-cols-2 gap-12 items-center">
        <div className="relative">
          <div className="absolute -inset-3 bg-fsc-azul rounded-2xl rotate-2" />
          <img src={img} alt="" className="relative rounded-2xl w-full h-[480px] object-cover border-2 border-fsc-negro" />
        </div>
        <div>
          <div className="font-cursive text-2xl text-fsc-azul-oscuro">misión</div>
...
function Pill({ icon, title, body }) {
  return (
    <div className="border-2 border-fsc-negro bg-white rounded-xl p-4 fsc-card-shadow">
      <div className="text-fsc-azul">{icon}</div>
...
export function ContactBlock({ s = {} }) {
  return (
    <section className="bg-fsc-negro text-white py-16 border-t-4 border-fsc-azul">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-10 items-center">
        <div>
          <div className="font-cursive text-2xl text-fsc-azul">contáctanos</div>
          <h2 className="font-display text-4xl md:text-5xl tracking-wider">HABLEMOS</h2>
          <div className="h-1 w-16 bg-fsc-azul mt-3 mb-6"/>
          <ul className="space-y-3">
            {s.contact_email && (
              <li className="flex items-center gap-3"><Mail size={18} className="text-fsc-azul"/> <a href={`mailto:${s.contact_email}`} className="hover:text-fsc-azul">{s.contact_email}</a></li>
            )}
            {s.contact_phone && (
              <li className="flex items-center gap-3"><Phone size={18} className="text-fsc-azul"/> {s.contact_phone}</li>
            )}
          </ul>
          <div className="mt-6 flex items-center gap-3">
            {s.instagram && <a href={`https://instagram.com/${s.instagram.replace(/^@/, "")}`} target="_blank" rel="noreferrer" className="h-11 w-11 rounded-md border-2 border-fsc-azul text-fsc-azul hover:bg-fsc-azul hover:text-fsc-negro transition-colors flex items-center justify-center"><Instagram size={18}/></a>}
            {s.facebook && <a href={s.facebook.startsWith("http") ? s.facebook : `https://facebook.com/${s.facebook}`} target="_blank" rel="noreferrer" className="h-11 w-11 rounded-md border-2 border-fsc-azul text-fsc-azul hover:bg-fsc-azul hover:text-fsc-negro transition-colors flex items-center justify-center"><Facebook size={18}/></a>}
            {s.youtube && <a href={s.youtube.startsWith("http") ? s.youtube : `https://youtube.com/${s.youtube}`} target="_blank" rel="noreferrer" className="h-11 w-11 rounded-md border-2 border-fsc-azul text-fsc-azul hover:bg-fsc-azul hover:text-fsc-negro transition-colors flex items-center justify-center"><Youtube size={18}/></a>}
          </div>
        </div>
        <div className="flex justify-center">
          <div className="bg-fsc-negro border-2 border-fsc-azul p-10 rounded-2xl">
