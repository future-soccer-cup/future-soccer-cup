          ...
          return (
            <div key={q.id} className="bg-white border border-slate-200 rounded-lg p-5 grid md:grid-cols-12 gap-4 items-center" data-testid={`my-quote-${q.id}`}>
              ...
              <div className="md:col-span-2 text-sm">
                ...
                <div className="font-display text-2xl font-black text-blue-700 tabular-nums">${q.total_amount}</div>
              </div>
              <div className="md:col-span-2 text-xs text-slate-500">
                {new Date(q.created_at).toLocaleDateString("es", { dateStyle: "medium" })}
              </div>
              <div className="md:col-span-2 text-right">
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${s.color}`}>{s.label}</span>
              </div>
            </div>
          );
        })}
