...
export default function TeamDetail() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12" data-testid="team-detail-page">
      <Link to="/equipos" className="text-xs uppercase tracking-widest font-bold text-blue-700">← Equipos</Link>

      <div className="mt-4 flex items-center gap-6 pb-8 border-b border-slate-200">
        <div className="h-24 w-24 rounded-2xl flex items-center justify-center text-4xl font-display font-black" style={{ background: team.color || "#1d4ed8", color: "#fff" }}>
          {team.logo_url ? <img src={team.logo_url} alt={team.name} className="h-full w-full object-contain p-2" /> : team.name[0]}
        </div>
        <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {players.map((p) => (
          <Link key={p.id} to={`/jugadores/${p.id}`} className="bg-white border border-slate-200 rounded-lg p-4 flex items-center gap-3 hover:border-blue-700" data-testid={`player-link-${p.id}`}>
            ...
            {p.photo_url ? <img src={p.photo_url} alt={p.name} className="h-12 w-12 rounded-full object-cover" /> : <div className="h-12 w-12 rounded-full bg-slate-100 flex items-center justify-center text-sm font-bold">{p.name[0]}</div>}
